...
# Check issues by looking up in a dictionary of potential issue keywords
...
def query_issue_type(query):
    ...
    issues_found = check_issues(query_terms)
    ...
    if issues_found:
        action = issue_handlers.get(issues_found[0], lambda: "Unknown issue. Please provide more details.")
        return action()
    else:
        return retrieve_relevant_records(" ".join(query_terms))
